...
function BaHuangGong takes nothing returns nothing
	...
	local timer t = null
	local integer gailv = 6 // 触发八荒加属性概率
...
function LiuYangZhang takes nothing returns nothing
	local unit u = GetTriggerUnit()
	local location loc = GetUnitLoc(u)
	call WuGongShengChong(GetTriggerUnit(), 'A02H', 120)
...
// 天山六阳掌+反两仪刀法+太玄经=被动触发
function IsLiuYangBeiDong takes nothing returns boolean
	return GetUnitAbilityLevel(GetAttacker(), 'A02H') >= 1 and GetUnitAbilityLevel(GetAttacker(), 'A0D8') >= 1 and GetUnitAbilityLevel(GetAttacker(), 'A07H') >= 1 and GetRandomInt(1, 100) <= 15
endfunction
