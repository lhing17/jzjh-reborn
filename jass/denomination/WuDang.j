...
function oD takes nothing returns nothing
	...
	call WuGongShengChong(u, 'A08Q', 180.)
	set t = null
	set u = null
	set p = null
	set loc = null
...
function chunYangRemoveAgi takes nothing returns nothing
	local timer t = GetExpiredTimer()
	local unit u = LoadUnitHandle(YDHT, GetHandleId(t), 0)
	local integer count = LoadInteger(YDHT, GetHandleId(t), 1)
	call ModifyHeroStat(1, u, 1, count)
...
function rD takes nothing returns nothing
	local unit u = GetTriggerUnit()
	local timer t = CreateTimer()
	local integer count = 50 + 10 * GetUnitAbilityLevel(u, 'A08Q')
	call ModifyHeroStat(1, u, 0, count)
	call SaveUnitHandle(YDHT, GetHandleId(t), 0, u)
	call SaveInteger(YDHT, GetHandleId(t), 1, count)
	call TimerStart(t, 60, false, function chunYangRemoveAgi)
	set t = null
	set u = null
endfunction
